...
namespace game
{
	...
	private:
		...
		ci_ext::Vec3f scale;
		int alpha;
		ci_ext::Vec3i rgb; //今は3色まとめておく
		/*nowpos_*/

		//-------------update------------//
		int frames_;										// アニメカウンタ
		ANIMSTATE state_anim;
		DOSTATE state_do;
		bool selected_;										// 選択状態
		bool RGBFLAG;
		ci_ext::Vec3f velocity_;
		/*nowpos_;*/
		ci_ext::Vec3f nextpos_;
	...
	private:
		...
		*/
		...
		@return							DIRECT
		*/
		DIRECTION TranceDirection(const ci_ext::Vec3f &p_nextpos, const ci_ext::Vec3f &p_nowpos);
		/*
		@brief							移動方向フラグをセット
		@return							DIRECT
		*/
		...
		*/
		...
		void Calc_Summon(const ci_ext::Vec3i &masu, const int in_frames);
		void Calc_Rotation(const ci_ext::Vec3i &masu, const int in_frames);
		void Calc_Parallel(const ci_ext::Vec3i &masu, const int in_frames);
		void Calc_Fall(const int in_frames);
	...
	public:
		...
		void Dice::receiveMsg(std::weak_ptr<Object>& sender, const std::string& msg) override;
